...
def get_dependence_sets(mu, j, n):
    ...
    for t in range(n-1, -1, -1):
        if mu == 0:
            R_prev = set(R)          # U_k^{t+1} depends on U_k^t
        elif mu == -1:
            R_prev = {i + 1 for i in R}   # U_k^{t+1} depends on U_{k+1}^t
        elif mu == -2:
            R_prev = set(R) | {i + 1 for i in R}  # depends on U_k^t and U_{k+1}^t
        else:
            raise ValueError("mu can only be 0, -1, -2")
        R = R_prev
        sets[t] = R
    return sets
...
if __name__ == "__main__":
    ...
    plt.suptitle('The numerical domains of dependence of the grid point $(x_j, t_3)$ for the upwind method', fontsize=14)
    plt.tight_layout()
    plt.savefig('figure/Exercise14.103.pdf')